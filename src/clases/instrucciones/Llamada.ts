...
export default class Llamada implements Instruccion, Expresion {
  ...
  validar_parametros(parametros_llamada: Array<Expresion>, parametros_funcion: Array<Simbolos>, controlador: Controlador, ts: TablaSimbolos, ts_local: TablaSimbolos) {
    ...
    if (parametros_llamada.length == parametros_funcion.length) {
      ...
      for (let i = 0; i < parametros_llamada.length; i++) {
        ...
        aux = parametros_funcion[i] as Simbolos;
        aux_id = aux.identificador;
        aux_tipo = aux.tipo.type; // ENTERO, DOBLE 
        ...
        if (aux_tipo == aux_exp_tipo) {
          ...
          let simbolo = new Simbolos(aux.simbolo, aux.tipo, aux_id, aux_exp_valor);
